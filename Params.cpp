...
Params::Params(const int argc, char *const argv[]) : pathname(nullptr)
{
    ...
    for (int j = 0; j < argc; j++)
        command += string(argv[j]) + " ";
    for (;;)
    {
        // Process the command-line switches
        ch = getopt_long(argc, argv, "vo:", longOpts, &code);
        if (ch == -1)
            break; // No more recognized short switches.
        switch (ch)
        {
        case 'v':
            switches[0] = true;
        ...
        case 0:
        {
            ...
            if (longOpts[code].name == "dir")
                pathname = optarg;

            if (longOpts[code].name == "dir")
                chdir(pathname = optarg);
            else if (longOpts[code].name == "ignore")
                switches[1] = true;
    ...
    out.open(string(filename), ios::app);
